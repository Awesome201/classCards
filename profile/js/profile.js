...
let deleteUser = document.getElementById('reset');
...
function addNewUser(event) {
  ...
  let newUserAvatar = event.target.userAvatar.value;
  let newUserHobby = event.target.userHobby.value;
  let newUserColor = event.target.userColor.value;
  let newKnownLanguage = [];
  let newInterestedLanguage = [];
  ...
  for(let j = 0; j < event.target.learnLanguage.length; j++){
  ...
  for(let k in Profile.allProfiles) {
...
function editUser () {
  let langTotal = 9;
  let placeHolderInfo = JSON.parse(localStorage.getItem('newUserName'));
